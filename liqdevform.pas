...
uses
  Classes, SysUtils, db, FileUtil, DBDateTimePicker, LResources, Forms,
  Controls, Graphics, Dialogs, ExtCtrls, ComCtrls, Buttons, StdCtrls, DbCtrls,
  DBGrids, LR_Class, LR_DBSet, frametools, LCLType, IBQuery, IBCustomDataSet,
  IBUpdateSQL;
...
type
  ...
  Tliqdevfrm = class(TForm)
    ...
    rptMOVqry: TIBQuery;
    rptMOVqryBODEGA: TIBStringField;
    rptMOVqryCANTIDAD: TIBBCDField;
    rptMOVqryCODPROD: TIBStringField;
    rptMOVqryDOC: TIntegerField;
    rptMOVqryEMPRESA: TIBStringField;
    rptMOVqryESTATUS1: TStringField;
    rptMOVqryFECHA: TDateTimeField;
    rptMOVqryITEM: TIntegerField;
    rptMOVqryPLUMOVSUCURSAL: TIntegerField;
    rptMOVqryPLUPRODUCTO: TIntegerField;
    rptMOVqryPLUTIPO: TIntegerField;
    rptMOVqryPRODUCTO: TIBStringField;
    rptMOVqryTIPO: TIBStringField;
    rptMOVqrytipodoc1: TStringField;
    rptMOVqryTIPO_PEDIDO: TIBStringField;
    rptMOVqryUNITARIO: TFloatField;
    rptMOVqryUSUARIO: TIBStringField;
    rptMOVqryVENDEDOR: TIBStringField;
...
begin
  if MSUCURSALMQRYpost.Value ='F' then
  begin
     if MessageDlg('El DOcumento no esta Procesado, si sale de la Pantalla El Documento sera Borrado**, Desea Continuar...?',
      mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      begin
...
begin
 ...
 try
  ...
  SHOWMESSAGE(msucursalmqryplumovsucursal.AsString);
...
procedure Tliqdevfrm.MSUCURSALDQRYAfterDelete(DataSet: TDataSet);
begin
   if MSUCURSALMQRYpost.Value ='T' then
    exit;
   IF MSUCURSALMQRY.IsEmpty THEN
...
procedure Tliqdevfrm.MSucursalMQRYBeforePost(DataSet: TDataSet);
begin
...
begin
   ...
   try
     ...
     if outil.ValidaLiq(msucursalmqrypluempleados.Value,msucursalmqryfecha.Value) then
     begin
        showmessage('Vendedor ya posee Liquidacion en esta fecha, ya no puede Devolver productos!!');
        exit;
     end;
